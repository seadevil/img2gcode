  ...
  proc Frame {w} {
	frame $w -relief ridge -bd 2
  	...
  	grid {*}[labelThing -path $w -name step1 -text "Step-1 :" -cmd "Load Image..." image::Load...] -sticky ew
   	grid {*}[labelThing -path $w -name step2 -text "Step-2 :" -cmd "Choose gcode output file..." {FIXME_2}] -sticky ew
   	grid {*}[labelThing -path $w -name step3 -text "Step-3 :" -cmd "Choose gcode image size..." {FIXME_3}] -sticky ew
	grid {*}[labelThing -path $w -name step4 -text "Step-4 :" -cmd "Set machine/gcode options" {FIXME_4}] -sticky ew
